...
def handle_object_appeared(event_type, event):
    ...
    match_object = re.search("(.*?)( )(object_type)(=)(.*?)( )(.*)", str(event.obj))
    
    if match_object:
        print ("DO WE HAVE A MATCH?")
        print ("G0: " + match_object.group(0))
        print ("G1: " + match_object.group(1))
        print ("G2: " + match_object.group(2))
        print ("G3: " + match_object.group(3))
        print ("G4: " + match_object.group(4))
        print ("G5: " + match_object.group(5))
        print ("G6: " + match_object.group(6))
        print ("G7: " + match_object.group(7))

        if match_object.group(5) == "CustomObjectTypes.CustomType00" and ssd_flag_found == False:
            #robot.say_text("That SSD Drive was dead on arrival.")
            #robot.say_text("It belongs in the garbage")
            #robot.say_text("Bye Bye SSD Drive!")

            print ("SSD DRIVE APPEARED")
            robot.say_text("I see the drive")

            #robot.anim.play_animation('anim_feedback_meanwords_01')
            ssd_flag_found = True
